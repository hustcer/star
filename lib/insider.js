...
let queryTopAsync = function queryTopAsync(type){
    ...
    return request(option)
        .spread((resp, res) => {
            ...
            from = moment().subtract(Number(res.condition.span), 'months').format(ft.inDate);
    ...
    return Promise
        ...
        .then(top => {
            ...
            console.log(' '.repeat(16) + 'Done!'.header, '耗时:', `${end - start} ms,`.em, '最近',
                        `${Number(top.option.qs.span)}`.em, '月', (type === 'BV')?'买入总额前':'卖出总额前',
