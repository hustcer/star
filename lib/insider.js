...
let padding = function(str, length, direction){
...
let displayDetail = function(code, summary, tradings){
    ...
    if(market === 'sz'){
        console.log((printf(conf.insider.sz.TH, '证券简称', '代码', '交易人', '变动股数', '均价', '结存股数', '填报日期', '变动原因', '高管姓名', '关系', '职务') + ' '.repeat(5)).underline);
        _.each(tradings, t => {
            t.NAME          = padding(t.NAME, 3);
            t.CHANGE_NUM    = numeral(t.CHANGE_NUM).format();
            t.HOLDSTOCK_NUM = numeral(t.HOLDSTOCK_NUM).format();
            t.FORM_DATE     = t.FORM_DATE.replace(/\-/g, '/');
            t._INSIDER      = padding(t._INSIDER, 3);
            console.log(printf(conf.insider.sz.TD, t.COMPANY_ABBR, t.COMPANY_CODE, t.NAME, t.CHANGE_NUM,
                               t.CURRENT_AVG_PRICE, t.HOLDSTOCK_NUM, t.FORM_DATE, t.CHANGE_REASON, t._INSIDER, t._RELATION, t.DUTY));
        });
        return false;
    }
    console.log((printf(conf.insider.sh.TH, '证券简称', '代码', '交易人', '变动股数', '均价', '结存股数', '变动日期', '填报日期', '变动原因', '职务') + ' '.repeat(10)).underline);
    _.each(tradings, t => {
        t.NAME          = padding(t.NAME, 3);
        t.CHANGE_NUM    = numeral(t.CHANGE_NUM).format();
        t.HOLDSTOCK_NUM = numeral(t.HOLDSTOCK_NUM).format();
        ...
        t.FORM_DATE     = t.FORM_DATE.replace(/\-/g, '/');
        console.log(printf(conf.insider.sh.TD, t.COMPANY_ABBR, t.COMPANY_CODE, t.NAME, t.CHANGE_NUM,
                           t.CURRENT_AVG_PRICE, t.HOLDSTOCK_NUM,t.CHANGE_DATE, t.FORM_DATE, t.CHANGE_REASON, t.DUTY));
    });
};
...
let queryInsider = function(code) {
    ...
    request(option, function(e, r, body){
        ...
        if(market === 'sz'){
            ...
            tradings = parseJsonP(body).result;
            _.each(tradings, t => {
        ...
        console.log(' '.repeat(35) + 'Done!'.header, '操作耗时:', ((end - start) + ' ms').em, ' '.repeat(18),'By TraceInvest.com\n' );
    });
};
