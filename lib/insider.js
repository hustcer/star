 ...
 * @return {Object}          Trading summary.
 */
let calcLatestSummary = function(tradings){
    let summary      = {};
    let summaries    = [];
...
/**
 * Display latest insider trading summary
 * @param  {Object} summary  Trading summary
 */
let displayLatestSummary = function(summary){
    // console.log(JSON.stringify(summary, null, 4));
    console.log(('\n董监高近期交易信息, 从: ' + from + ', 到: '+ to + ' '.repeat(50) + '\n').em.underline);

    _.each(summary, s => {
        console.log('  净增持股数：' , printf('%9s', s.netBuyShare), ' '.repeat(7),
                    '净增持额：'  , printf('%9s', s.netBuyCost), ' '.repeat(7),
                    '公司：'.em, s.company.em);
        console.log('  总增持股数：' , printf('%9s', s.buyShare), ' '.repeat(7),
                    '增持均价：'  , printf('%9s', s.buyPrice), ' '.repeat(9),
                    '总增持额：'  , printf('%9s', s.buyCost));
        console.log('  总减持股数：' , printf('%9s', s.sellShare), ' '.repeat(7),
                    '减持均价：'  , printf('%9s', s.sellPrice), ' '.repeat(9),
                    '总减持额：'  , printf('%9s', s.sellProfit));
        console.log((' '.repeat(90)).gray.underline);
    });

    if(cmd.showDetail){
        console.log(('\n交易详情:'+' '.repeat(102)).header.underline);
    }
};
...
let displayTradings = function(market, tradings){
    if(market === 'sz'){
        console.log((printf(conf.insider.sz.TH, '证券简称', '代码', '交易人', '变动股数', '均价',
                                                '结存股数', '填报日期', '变动原因', '高管姓名', '关系',
                                                '职务') + ' '.repeat(5)).underline);
        _.each(tradings, t => {
            // Fix the padding of company name with 3 characters
            if(t.COMPANY_ABBR.length === 3){ t.COMPANY_ABBR = ' '.repeat(4) + t.COMPANY_ABBR; }
            ...
            console.log(printf(conf.insider.sz.TD, t.COMPANY_ABBR, t.COMPANY_CODE, t.NAME,
                               t.CHANGE_NUM, t.CURRENT_AVG_PRICE, t.HOLDSTOCK_NUM, t.FORM_DATE,
...
let displayDetail = function(code, summary, tradings){
    ...
    console.log('净增持股数：' , printf('%7s', summary.netBuyShare), ' '.repeat(9),
                '净增持额：'  , printf('%9s', summary.netBuyCost));
    console.log('总增持股数：' , printf('%7s', summary.buyShare), ' '.repeat(9),
                '增持均价：'  , printf('%6s', summary.buyPrice), ' '.repeat(9),
                '总增持额：'  , printf('%9s', summary.buyCost));
    console.log('总减持股数：' , printf('%7s', summary.sellShare), ' '.repeat(9),
                '减持均价：'  , printf('%6s', summary.sellPrice), ' '.repeat(9),
                '总减持额：'  , printf('%9s', summary.sellProfit));
...
let queryInsider = function(code, cb) {
    ...
    request(option, function(e, r, body){
        ...
        if(e || (r && r.statusCode !== 200)){
            if(r.statusCode === 408){ console.error(MSG.SYSTEM_BUSY.error); return false; }
        ...
        console.log(' '.repeat(111).underline.yellow);
...
/**
 * Get query latest insider trading options
 * @param  {Function} callback Callback
 */
let getQueryLatestOptions = function(callback){
    let option = conf.insider;
    let spanD  = cmd.span || '10d';
    option.sz.qs.CATALOGID = '';
...
let querySH = function(option, callback){
    ...
    request(option.sh, function(e, r, body){
        ...
        let summary = calcLatestSummary(tradings);
        displayLatestSummary(summary);

        if(cmd.showDetail){ displayTradings('sh', tradings); }

        let end      = new Date().getTime();
        console.log(' '.repeat(111).underline.yellow);

        console.log(' '.repeat(35) + 'Done!'.header, '操作耗时:', ((end - shStart) + ' ms').em,
                    ', 总交易记录：' + (tradings.length+'').em,
                    ' '.repeat(15),'By TraceInvest.com\n' );
...
let querySZ = function(option, callback){
    async.eachSeries(['1801_sme','1801_nm','1801_zb'], function(cat, cb){
        let szStart   = new Date().getTime();
        option.sz.qs.CATALOGID   = cat;
        option.sz.qs.tab1PAGENUM = cmd.page ? cmd.page : 1;
        request(option.sz, function(e, r, body){
            let tradings = [];
            if(e || (r && r.statusCode !== 200)){
                if(r.statusCode === 408){ console.error(MSG.SYSTEM_BUSY.error); cb(e||r); return false; }
                console.error(MSG.REQUEST_ERROR, printf(JSON.stringify(e||r, null, 4).error));
                cb(e||r);
                return false;
            }
            ...
            if(cmd.showDetail){ displayTradings('sz', tradings); }

            let end  = new Date().getTime();
            console.log(' '.repeat(111).underline.yellow);

            console.log(' '.repeat(35) + 'Done!'.header, '操作耗时:', ((end - szStart) + ' ms').em,
                        ', 总交易记录：' + (tradings.length+'').em,
                        ' '.repeat(15),'By TraceInvest.com\n' );
            cb(null);
        });
...
let querySHLatest = function(){
    ...
    async.waterfall([getQueryLatestOptions, querySH], function(err){
...
let querySZLatest = function(){
    ...
    async.waterfall([getQueryLatestOptions, querySZ], function(err){
