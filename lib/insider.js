...
let getSzPagingInfo = function(html){
    ...
    if(text === MSG.NO_DATA_FROM_SZ){ return 0; }

    let pgUrl = $('input.cls-navigate-next', this).attr('onclick');
    if(!pgUrl) { return 1; }

    let pgIdx      = pgUrl.indexOf('&tab1PAGECOUNT=') + '&tab1PAGECOUNT='.length;
    let totalSzPg  = _.parseInt(pgUrl.substr(pgIdx));

    let totalIdx   = pgUrl.indexOf('&tab1RECORDCOUNT=') + '&tab1RECORDCOUNT='.length;
    let totalCount = _.parseInt(pgUrl.substr(totalIdx));
...
let displayTradings = function(market, tradings){
    ...
    console.log((printf(conf.insider[MISC_QUERY_KEY].TH, '证券简称', '代码', '交易人', '变动股数', '均价',
                                                         '结存股数', '变动日期', '变动原因', '职务') +
                                                         ' '.repeat(10)).underline);
    _.each(tradings, t => {
        t.COMPANY_ABBR  = t.COMPANY_ABBR.padOutput(8, 'left');
        ...
        console.log(printf(conf.insider[MISC_QUERY_KEY].TD, t.COMPANY_ABBR, t.COMPANY_CODE, t.NAME,
                           t.CHANGE_NUM, t.CURRENT_AVG_PRICE, t.HOLDSTOCK_NUM, t.CHANGE_DATE,
                           t.CHANGE_REASON, t.DUTY));
    });
...
let getQueryMiscOption = function(){
    let option = conf.insider[MISC_QUERY_KEY];
...
let querySZ = function(option, callback){
    ...
    request(option.sz, function(e, r, body){
        ...
        console.log(' '.repeat(25) + 'Done!'.header, '耗时:', ((end - szStart) + ' ms').em,
                    ', 总记录：' + (pgInfo.totalCount+'').em, '当前:', idx + '-' + (idx+tradings.length-1),
                    '页码:', option.sz.qs.tab1PAGENUM + '/' + pgInfo.totalPg,
                    ' '.repeat(8),'By TraceInvest.com\n' );
        callback(null);
    });
};
    ...
    return request(option)
        .spread(function(resp, res){
            ...
            console.log(' '.repeat(18) + 'Done!'.header, '耗时:', ((end - start) + ' ms').em,
                        ', 总记录：' + (res.total+'').em, '当前:', idx + '-' + (idx+tradings.length-1),
                        '页码:', option.qs.page + '/' + Math.ceil(res.total/option.qs.limit),
