...
let displayMiscSummary = function(summary){
    ...
    _.each(summary, s => {
        console.log('  净增持股数：', _.padRight(s.netBuyShare, 18),
                    '净增持额：', _.padRight(s.netBuyCost, 18),
                    '公    司：'.em, s.company.em);
        console.log('  总增持股数：', _.padRight(s.buyShare, 18),
                    '增持均价：', _.padRight(s.buyPrice, 18),
                    '总增持额：', s.buyCost);
        console.log('  总减持股数：', _.padRight(s.sellShare, 18),
                    '减持均价：', _.padRight(s.sellPrice, 18),
                    '总减持额：', s.sellProfit);
...
let displayTradings = function(market, tradings){
    let headers = {COMPANY_ABBR:'证券简称', COMPANY_CODE:'代码', NAME:'交易人',
...
let displayDetail = function(code, summary, tradings){
    ...
    console.log('净增持股数：', _.padRight(summary.netBuyShare, 18),
                '净增持额：'  , _.padRight(summary.netBuyCost, 20));
    console.log('总增持股数：', _.padRight(summary.buyShare, 18),
                '增持均价：', _.padRight(summary.buyPrice, 20),
                '总增持额：', summary.buyCost);
    console.log('总减持股数：', _.padRight(summary.sellShare, 18),
                '减持均价：', _.padRight(summary.sellPrice, 20),
...
let queryInsider = function(code, cb) {
    ...
    request(option, function(e, r, body){
        ...
        if(market === 'sz'){
            ...
            console.log(' '.repeat(35) + 'Done!'.header, '耗时:', `${end - start} ms`.em, ', 总记录：' +
                        `${tradings.length}`.em, ' '.repeat(15), 'By TraceInvest.com\n' );
        }
...
let querySZ = function(option, callback){
    ...
    request(option.sz, function(e, r, body){
        ...
        console.log(' '.repeat(25) + 'Done!'.header, '耗时:', `${end - start} ms`.em + ', 总记录：' +
                    ...
                    `${option.sz.qs.tab1PAGENUM} / ${pgInfo.totalPg}`.em, ' '.repeat(8), 'By TraceInvest.com\n' );
        callback(null);
    });
};
...
let displayTopList = function(topData){
    ...
    let data    = [];
    let headers = {COMPANY_ABBR:'公司简称', COMPANY_CODE:'证券代码', meanPrice:'交易均价',
    ...
    _.each(topData.data, t => {
        ...
        t.meanPrice    = numeral(t.meanPrice).format(ft.money);
    ...
    return Promise
        ...
        .then((top) => {
            ...
            console.log(' '.repeat(16) + 'Done!'.header, '耗时:', `${end - start} ms,`.em, '最近',
                        `${parseInt(top.option.qs.span)}`.em, '月', (type === 'BV')?'买入总额前':'卖出总额前',
