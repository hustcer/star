...
let calcSummary = function(tradings){
    ...
    summary.buyPrice    = numeral(summary.buyPrice).format(conf.fmt.flot);
    summary.sellPrice   = numeral(summary.sellPrice).format(conf.fmt.flot);
...
let displayDetail = function(code, summary, tradings){
    ...
    console.log(('\n董监高近期交易信息，股票代码：'+ code + ' '.repeat(70)).em.underline);
    ...
    console.log(('\n交易详情:'+' '.repeat(90)).header.underline);
};
