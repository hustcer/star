...
let printResults = function(){
    ...
    let headers = {name:'公司', code:'代码', price:'当前价', incPct:'涨跌%',
                   cheap:'买点', expensive:'卖点', target:'目标价', pct:'上涨空间',
                   star:'星级', capacity:'总市值', pe:'P/E', pb:'P/B', comment:'备注'};

    let hiddenCol  = ['hold', 'watch', 'close', 'bdiff', 'sdiff'];
    let ignoreKeys = ['name', 'code', 'comment', 'star'];
    let option     = {
        align            : 'right',
        columnSplitter   : '  ',
        config: {comment : {align: 'left'}, star: {align: 'left'}},
        headingTransform : h => headers[h].em.underline
    };

    _.each(page, function(s){
        _.each(hiddenCol, c => delete s[c]);
        _.each(s, (v, k) => {
    ...
    console.log(columnify(page, option));
};
